...
function setupProfileMenu() {
  ...
  const modelLabel = $("profile-model-label");
  ...
  modelSelect.addEventListener("change", () => {
    ...
    addLogEntry(`Switched model preset: ${text}`);
  });

  toggleThemeBtn.addEventListener("click", () => {
    // For now, just toggle label & log; full light theme would require different palette.
    theme = theme === "dark" ? "dark (locked)" : "dark";
    toggleThemeBtn.querySelector("span:nth-child(2)").textContent = theme;
    addLogEntry("Theme toggle clicked (dark theme enforced).");
  });
}
